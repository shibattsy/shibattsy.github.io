...
var simulation_cycle = 8; // cycle time (ms)
...
function run_minimax() {
   ...
   previous_node_id = -1;
   step_minimax = 0;
   setinterval_id = setInterval(minimax_search,simulation_cycle);
}
function minimax_search() {
   ...
   minimax_value();
   if (previous_node_id >= 0)
      enter_node_mm(previous_node_id * node_size);
...
// changes node color to LightGreen and updates node info
function enter_node_mm(idx_node) {
   var canvas = document.getElementById('canvasab');
   var context = canvas.getContext('2d');
   draw_node(context, idx_node, "LightGreen");
   ...
   if (gameTreeArray[idx_node + slot_node_level] % 2 == 0)
      context.fillText(step_minimax, cx-5, cy+10); // prints step number for max player 
   else
      context.fillText(step_minimax, cx-5, cy-5); // prints step number for min player
...
function alphabeta_search() {
   ...
   alphabeta_value();
   if (previous_node_id >= 0)
      enter_node_ab(previous_node_id * node_size);
   if (current_node_id==-1 && simulation_mode==1) {
      clearInterval(setinterval_id); // ends simulation
      step_minimax=0;
      simulation_mode=0;
...
/* changes node color to LightGreen and updates node info*/
function enter_node_ab(idx_node) {
   var canvas = document.getElementById('canvasab');
   var context = canvas.getContext('2d');
   draw_node(context, idx_node, "LightGreen");
   var cx = gameTreeArray[idx_node + slot_x_pos];
   ...
   if (gameTreeArray[idx_node+slot_number_childs]>0) {
      ...
      context.fillStyle = "black";
      context.fillRect(xx+2, cy-10, tx*4, 34);
      // updated node info
      context.fillStyle = "white";
      context.fillText("\u237a="+gameTreeArray[idx_node+slot_alpha], xx+13, cy-2); // alpha value
      context.fillText("\u03b2="+gameTreeArray[idx_node+slot_beta], xx+13, cy+8); // beta value
      context.fillText("v="+gameTreeArray[idx_node+slot_value], xx+13, cy+18); // node value 
...
function draw_node(context, idx_node, fill_color) {
   ...
   if ((gameTreeArray[idx_node + slot_value]>-1000) && (gameTreeArray[idx_node + slot_value]<1000))
      context.fillText(gameTreeArray[idx_node + slot_value], cx-4, cy+ty-9);
}
...
function create_game_tree() {
   ...
   console.log(arrayOfStringsInnerNodes,arrayOfStringsLeafNodes );
